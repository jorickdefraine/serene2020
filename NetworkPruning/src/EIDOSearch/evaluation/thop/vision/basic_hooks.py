...
def count_convNd(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    ...
    zeroed_weights = m.weight.numel() - m.weight.nonzero().size(0)
    # print("zero: {}".format(zeroed_weights))

    # N x (Cout x (Cin x Kw x Kh) - zeroed_weights) x H x W
    op_per_Cin = m.in_channels // m.groups * kernel_ops  # + bias_ops
    out_size = torch.zeros(y.size()[2:]).numel()
    Cout = y.size()[1]
    op_per_Cout = Cout * op_per_Cin - zeroed_weights
    total_ops = y.size()[0] * op_per_Cout * out_size
    # print(total_ops)
...
def count_linear(m, x, y):
    ...
    zeroed_weights = m.weight.numel() - m.weight.nonzero().size(0)
    # print("zero: {}".format(zeroed_weights))
    total_ops = total_mul * num_elements - zeroed_weights
    # print(total_ops)
