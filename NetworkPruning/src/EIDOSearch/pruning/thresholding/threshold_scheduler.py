...
class Scheduler(object):
    ...
    def __init__(self, model, layers, valid_loader, loss_function, twt, pwe):
        """
        ...
        for n_m, mo in self.model.named_modules():
            if isinstance(mo, self.layers):
                for n_p, p in mo.named_parameters():
                    if "weight" in n_p:
                        data = torch.abs(p).clone().detach()
                        sum_w += data[data != 0].sum()
                        elements += data[data != 0].numel()

        return sum_w / elements
    ...
    def _find_threshold_with_bisection(self):
        ...
        _, _, loss_base_model = test_model(self.model, self.loss_function, self.valid_loader)
        self.bound = loss_base_model + (loss_base_model * self.twt)

        magnitude_threshold(self.model, self.layers, mean_T)
        _, _, loss_threshold_model = test_model(self.model, self.loss_function, self.valid_loader)
